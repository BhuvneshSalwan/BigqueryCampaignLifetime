...
public class BigQueryJavaGettingStarted {
  ...
  private static final String CLIENTSECRETS_LOCATION = "C:\\Program Files\\Java\\projects\\BigqueryCampaignLifetime\\target\\client_secret_154911048506-ign21bljsc6jmrdvrd84tqt3a6omk5be.apps.googleusercontent.com.json";
 ...
 * @throws ParseException 
  ...
  {
 	  String querySql = "SELECT parse_campaign_id, impressions, clicks, conversions, conversion_inr, status FROM ["+PROJECT_ID+":dsp_processed.campaign_lifetime]";
  ...
  private static void displayQueryResults(Bigquery bigquery,
    ...
    if(totRows != 0){
    	...
    	for (TableRow row : rows) {
            ...
            client.setCampaign_id((String)list.get(0).getV());
            ...
            client.setConversions(Integer.parseInt(list.get(3).getV().toString()));
            client.setConversion_inr(Float.parseFloat(list.get(4).getV().toString()));
            client.setStatus(Integer.parseInt(list.get(5).getV().toString()));
            
        	System.out.println(client.getString());
        	
            if(null != client.getCampaign_id()){
        	//if(null != list.get(0).getV()){
        			        		
         		UpdateResult result = null;
        		
        	    result = MongoDAO.getMongoObject().updateOne(Filters.eq("_id", client.getCampaign_id()), Updates.combine(Updates.set("impressions", client.getImpressions()),Updates.set("clicks", client.getClicks()),Updates.set("conversions", client.getConversions()),Updates.set("conversion_value", client.getConversion_inr()),Updates.set("status", client.getStatus())));
